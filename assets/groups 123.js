...
function joinGroup(code) {
    ...
    if (group.joinRequests && group.joinRequests.some(r => r.name === userName)) {
        alert('You have already sent a join request to this group.');
        return;
    }

    // Show group info before joining
    const description = group.description ? `\nDescription: ${group.description}` : '';
    const confirmMessage = `Request to join "${group.name}"?\n${group.members.length} members${description}\n\nYour request will be sent to the admin for approval.`;

    if (!confirm(confirmMessage)) {
        return;
    }

    // Add to join requests instead of directly joining
    ...
    alert('✓ Your join request has been sent! The group admin will review it soon.');
}
...
function loadGroupInfo() {
    ...
    if (code && groups[code]) {
        ...
        if (group.description) {
            bannerStats.textContent += ` • ${group.description}`;
        }
